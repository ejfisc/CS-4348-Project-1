...
main.c is the main source file. Run that in the terminal using
```
gcc main.c -o main
```
main will be the output file. Then run
```
./main samplex.txt y
```
where x is the sample program number and y is the desired interrupt timer (every y instructions an interrupt will occur). Your interrupt timer must be greater than 1 (I used modulus to implement the timer interrupt, and every number is divisible by 1, so this will get stuck in an infinite loop).
*NOTE: When running sample 3, if you choose an interrupt timer that's a power of 2 (i.e. 2, 4, 8, 16, 32, ...) it gets stuck in an infinite loop and you'll have to use ctrl+c to quit the program. More details in summary.
